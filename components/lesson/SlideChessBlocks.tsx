...
function MissingSlideChessBlock({
  slug,
  expectedType,
}: {
  slug: string;
  ...
  return (
    <div className="my-4 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
      <p>
        ...
        <code className="rounded bg-black/5 px-1">{slug}</code> ({expectedType}).
      </p>
      <Link
...
function getBlock(
  slug: string,
  expectedType: LessonSlideChessBlockRow["type"],
): LessonSlideChessBlockRow | null {
  const ctx = useSlideChessContext();
  if (!ctx) return null;

  const block = ctx.blocksBySlug[slug];
  if (!block || block.type !== expectedType) return null;
  return block;
}

export function SlidePuzzle({ slug }: { slug: string }) {
  const block = getBlock(slug, "puzzle");
  if (!block) {
    return <MissingSlideChessBlock slug={slug} expectedType="puzzle" />;
  }
...
export function SlideBoard({ slug }: { slug: string }) {
  const block = getBlock(slug, "display-board");
  if (!block) {
    return <MissingSlideChessBlock slug={slug} expectedType="display-board" />;
  }
...
export function SlideAnalysis({ slug }: { slug: string }) {
  const block = getBlock(slug, "analysis-board");
  if (!block) {
    return (
      <MissingSlideChessBlock slug={slug} expectedType="analysis-board" />
    );
  }
